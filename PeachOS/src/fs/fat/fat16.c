...
struct fat_header
{
    ...
    uint16_t number_of_sectores;
    uint8_t media_type;
    uint16_t sectors_per_fat;
    uint16_t sectores_per_track;
                             ...
                             struct fat_directory* directory)
{
    ...
    directory->sectors_pos = root_dir_sector_pos;
...
int fat16_resolve(struct disk* disk)
{
    ...
    if(diskstreamer_read(
        stream, &fat_private->header, sizeof(fat_private->header)) != PEACHOS_ALL_OK) {
            res = -EIO;
            goto out;
    }
